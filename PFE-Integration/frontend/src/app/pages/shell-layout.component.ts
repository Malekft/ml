import { Component, HostListener, ChangeDetectorRef, OnInit, computed } from '@angular/core';
    ...
    <div class="shell">
          ...
          <div style="display:flex; align-items:center; gap:16px;">
            ...
            <div class="user-profile-wrapper" style="position: relative;">
              <div class="user-profile" (click)="toggleDropdown()">
                <div class="avatar" [ngClass]="userEmail | avatarColor : userAvatar">{{ userName | initials }}</div>
      ...
      <div *ngIf="isIncomingCall()" class="call-notification" style="position:fixed; top:24px; right:24px; width:320px; background:rgba(255, 255, 255, 0.9); backdrop-filter:blur(16px); border:1px solid rgba(255,255,255,0.4); border-radius:20px; box-shadow:0 20px 40px rgba(0,0,0,0.15); z-index:9999; padding:16px; display:flex; flex-direction:column; gap:16px; animation: slideIn 0.3s ease-out;">
        ...
        <div style="display:flex; gap:10px;">
           <button (click)="acceptCall()" style="flex:1; background:#22c55e; height:40px; border-radius:12px; display:flex; align-items:center; justify-content:center; gap:8px; border:none; color:white; cursor:pointer; font-weight:600; font-size:13px; transition:all 0.2s;">
              <i-lucide [name]="PhoneIcon" [size]="16"></i-lucide> Accepter
           </button>
           <button (click)="rejectCall()" style="flex:1; background:#ef4444; height:40px; border-radius:12px; display:flex; align-items:center; justify-content:center; gap:8px; border:none; color:white; cursor:pointer; font-weight:600; font-size:13px; transition:all 0.2s;">
              <i-lucide [name]="PhoneIcon" [size]="16" style="transform: rotate(135deg);"></i-lucide> Refuser
           </button>
        </div>
      </div>
      ...
      <div *ngIf="isCalling() && !isIncomingCall() && !isCallExpanded()" (click)="expandCallWidget()" style="position:fixed; bottom:24px; right:24px; width:auto; background:var(--bg-card); border:1px solid var(--border-color); border-radius:100px; box-shadow:0 10px 25px rgba(0,0,0,0.1); z-index:9999; padding:8px 16px 8px 8px; display:flex; align-items:center; gap:12px; cursor:pointer; animation: slideUp 0.3s ease-out; transition:transform 0.2s;">
        ...
        <button (click)="endCall(); $event.stopPropagation()" style="background:#ef4444; width:40px; height:40px; border-radius:50%; display:grid; place-items:center; border:none; color:white; cursor:pointer; transition:all 0.2s;">
           <i-lucide [name]="PhoneIcon" [size]="18" style="transform: rotate(135deg);"></i-lucide>
        </button>
      </div>
      ...
      <div *ngIf="isCalling() && !isIncomingCall() && isCallExpanded()" style="position:fixed; bottom:24px; right:24px; width:340px; background:var(--bg-card); border:1px solid var(--border-color); border-radius:20px; box-shadow:0 20px 40px rgba(0,0,0,0.2); z-index:9999; display:flex; flex-direction:column; overflow:hidden; animation: slideUp 0.3s ease-out;">
        ...
        <div style="padding:16px; display:flex; justify-content:center; gap:16px; background:var(--bg-app);">
           ...
           <button (click)="endCall()" style="background:#ef4444; width:48px; height:48px; border-radius:50%; display:grid; place-items:center; border:none; color:white; cursor:pointer; transition:all 0.2s;">
              <i-lucide [name]="PhoneIcon" [size]="20" style="transform: rotate(135deg);"></i-lucide>
           </button>
        </div>
      </div>
    </div>
...
export class ShellLayoutComponent implements OnInit {
  ...
  menu = [
    ...
    { icon: Megaphone, label: 'Annonces', path: '/communication', color: '#8b5cf6' },
    { icon: Users, label: 'Employés', path: '/employees', color: '#6366f1' },
    { icon: AlertTriangle, label: 'Absences', path: '/absences', color: '#f97316' },
    { icon: DoorOpen, label: 'Autorisations', path: '/autorisations', color: '#6366f1' },
    { icon: Calendar, label: 'Congés', path: '/leaves', color: '#10b981' },
    { icon: Ticket, label: 'Tickets', path: '/tickets', color: '#f59e0b' },
    { icon: Briefcase, label: 'Travail Supplémentaire', path: '/extra-work', color: '#ef4444' },
    { icon: ChartBarBig, label: 'BI Dashboard', path: '/bi-dashboard', color: '#06b6d4' },
    { icon: Globe, label: 'Analytics IA', path: '/analytics', color: '#22c55e' },
    { icon: MessageSquare, label: 'Messages', path: '/messages', color: '#ec4899' },
    { icon: Settings, label: 'Admin', path: '/admin', color: '#64748b' }
  ];
  ...
  constructor(
    ...
    const savedTheme = localStorage.getItem('hr_theme_' + email) || localStorage.getItem('theme') || 'light';
  ...
  private loadNotifications(): void {
    ...
    this.api.getNotifications(this.currentUserId).subscribe((res: any[]) => {
      ...
      this.unreadNotifs = this.notifications.filter((n: any) => n.unread).length;
      this.cdr.detectChanges();
    });
  }
  ...
  private getNotifUiMetadata(n: any): any {
    ...
    // Congés (Demandes Administrateur)
    if (msg.includes('demande') && (msg.includes('congé') || msg.includes('conge'))) {
      return { icon: Calendar, bg: 'var(--color-orange-bg)', color: 'var(--color-orange)', title: 'Demande Congé', path: '/leaves' };
    }

    // Absences (Admin)
    if (msg.includes('absence') || msg.includes('irrégulière')) {
      return { icon: AlertTriangle, bg: 'var(--color-red-bg)', color: 'var(--color-red)', title: 'Absence Irrégulière', path: '/absences' };
    }

    // Tickets (Nouveaux/Support)
    if (msg.includes('ticket') || msg.includes('support')) {
      return { icon: Ticket, bg: 'var(--color-blue-bg)', color: 'var(--color-blue)', title: 'Support IT', path: '/tickets' };
    }

    // Documents
    if (msg.includes('document') || msg.includes('dossier')) {
      return { icon: FileText, bg: 'rgba(34,197,94,0.1)', color: '#22c55e', title: 'Document', path: '/profile' };
    }

    // Travail Supplémentaire
    if (msg.includes('travail') || msg.includes('heures sup')) {
      return { icon: Briefcase, bg: 'rgba(79,70,229,0.1)', color: '#4f46e5', title: 'Travail Supp.', path: '/extra-work' };
    }
    
    // Messages
    if (msg.includes('message')) {
      return { icon: MessageSquare, bg: 'rgba(147,51,234,0.1)', color: '#9333ea', title: 'Nouveau Message', path: '/messages' };
    }

    // Annonces
    if (msg.includes('annonce')) {
      return { icon: Megaphone, bg: 'rgba(147,51,234,0.1)', color: '#9333ea', title: 'Annonce', path: '/communication' };
    }
    
    // Default
    return { icon: Info, bg: 'var(--bg-hover)', color: 'var(--text-muted)', title: 'Notification', path: '/' };
  }
  ...
  updateTitle(): void {
    ...
    const current = this.menu.find((m) => (m.path === '/' ? url === '/' : url.startsWith(m.path)));
    this.title = current?.label ?? 'Dashboard';
  }
